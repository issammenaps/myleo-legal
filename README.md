...
<script>
    window.MyleoWidgetConfig = {
        apiUrl: 'https://your-chatbot-domain.com/api',
        language: 'fr',
        rubrique: 'compte_client',            // Client account context
        theme: 'myleo',
...
*/15 * * * * cd /path/to/myleao && npm run sync >> /var/log/faq-sync.log 2>&1

# Or once per hour at minute 0
0 * * * * cd /path/to/myleao && npm run sync >> /var/log/faq-sync.log 2>&1
```
...
4. Set action: Start a program
   - Program: `node`
   - Arguments: `scripts/sync-faq-data.js`
   - Start in: `C:\path\to\myleao`

### External Database Structure

Your external FAQ table should have these columns (configurable via .env):
...
CREATE TABLE faqs (
    ...
    rubrique ENUM('produit', 'compte_client', 'tunnel_vente', 'general'),
    last_updated TIMESTAMP,
    qa_data JSON  -- Array of {code: "app.faq_entry", data: {question: "...", answer: "..."}}
);
```
